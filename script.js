...
function startTimer(time) {
    ...
    function timer() {
        if (time < 10) {
            timeCount.textContent = "0" + time; // Add leading zero if time is less than 10
        } else {
        ...
        if (time < 0) { // if timer is less than 0
            clearInterval(counter); // clear counter
            timeText.textContent = "Time End"; // change the time text to Time End
            // Disable all options
            disableOptions();
        }
    }
...
function disableOptions() {
    ...
    // Show the next button to allow the player to proceed manually
